...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Mock votes route for testing
  app.post('/api/polls/:pollId/mock-votes', isAuthenticated, async (req: any, res) => {
    try {
      const { pollId } = req.params;
      const userId = req.user.claims.sub;
      ...
      // Create mock votes for up to 15 friends (or however many friends exist)
      const friendsToVote = friends.slice(0, 15);
      
      for (let i = 0; i < friendsToVote.length; i++) {
        const friend = friendsToVote[i];
        // Distribute votes across options with some realistic bias
        let selectedOption;
        if (i < 5) {
          selectedOption = 0; // First 5 vote for option 0
        } else if (i < 9 && numOptions > 1) {
          selectedOption = 1; // Next 4 vote for option 1
        } else if (i < 12 && numOptions > 2) {
          selectedOption = 2; // Next 3 vote for option 2 if it exists
        } else if (i < 14 && numOptions > 3) {
          selectedOption = 3; // Next 2 vote for option 3 if it exists
        } else {
          selectedOption = Math.floor(Math.random() * numOptions); // Rest random
        }
        ...
        try {
          ...
          // Skip if friend already voted or other error
          continue;
        }
      }
      
      res.json({ 
        message: `Created ${mockVotes.length} mock votes from friends`,
        votes: mockVotes.length,
        totalFriends: friends.length
      });
    } catch (error) {
      console.error("Error creating mock votes:", error);
      res.status(500).json({ message: "Failed to create mock votes" });
    }
  });
