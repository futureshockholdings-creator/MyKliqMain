...
When users post URLs from supported platforms, the URLs are extracted from the text and displayed as branded preview cards instead of plain links. YouTube uses rich embeds with thumbnails and play buttons (`YouTubeEmbed.tsx`, `youtubeUtils.ts`). Instagram, Facebook, and TikTok display branded cards with platform logos, colors, and content type labels (`SocialMediaEmbed.tsx`, `socialMediaUtils.ts`). The extraction pipeline runs YouTube first, then social media, with remaining URLs rendered as clickable text via `LinkifyText`.
