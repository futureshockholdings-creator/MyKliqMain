...
When users post URLs from supported platforms, the URLs are extracted from the text and displayed as branded preview cards instead of plain links. YouTube uses rich embeds with thumbnails and play buttons (`YouTubeEmbed.tsx`, `youtubeUtils.ts`). Twitch uses rich embeds with stream/clip thumbnails, play buttons, and live badges (`TwitchEmbed.tsx`, `twitchUtils.ts`). Instagram, Facebook, and TikTok display branded cards with platform logos, colors, and content type labels (`SocialMediaEmbed.tsx`, `socialMediaUtils.ts`). All other URLs (news articles, blogs, etc.) display Open Graph-powered preview cards with article image, title, site name, and description via a server-side `/api/link-preview` endpoint (`LinkPreviewCard.tsx`, `linkPreviewUtils.ts`). The extraction pipeline runs YouTube first, then Twitch, then social media, then generic link previews, with remaining URLs rendered as clickable text via `LinkifyText`.
...
### PWA Install Prompt
A unified install prompt appears 3 seconds after page load for users visiting in a browser. Detects iOS vs Android/Desktop automatically. On Android/Desktop, a centered dialog with "Install App" button triggers the native `beforeinstallprompt`. On iOS, a top-of-page Smart App Banner (similar to Safari's native style) shows the app icon, name, description, and a green "Install" button with black text; tapping it expands the banner to show step-by-step Safari instructions (Share icon → Add to Home Screen → Add). The prompt hides if the app is already running in standalone/PWA mode and remembers dismissal for 7 days via localStorage.
